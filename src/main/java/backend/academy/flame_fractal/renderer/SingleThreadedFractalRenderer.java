...
public class SingleThreadedFractalRenderer implements FractalRenderer {
    ...
    public SingleThreadedFractalRenderer(Rect world, int maxIterations, int symmetry, List<Transformation> transformations, Map<Transformation, Color> transformationColors) {
        if (maxIterations <= 0) {
    ...
    public void render(FractalImage image, int samples, long seed) {
        ...
        System.out.printf("Однопоточный рендеринг завершен: %d выборок, %d потоков, время: %.2f секунд%n",
            samples, 1, (endTime - startTime) / 1e9);
    }
